...
from full3d import simulate  # your real 3D engine (Phase 1b lives here)
...
# Fast preview solver (analytical proxy)
...
# Preview hook used in a few tabs
def _get_sim_preview():
    rng = np.random.default_rng(int(st.session_state.rng_seed))
    # use current session_state as "state"
    ps = {k: st.session_state.get(k, defaults.get(k)) for k in defaults.keys()}
    return fallback_fast_solver(ps, rng)

# ------------------------ Engines ------------------------
# UPDATED: pass engine_type to backend (supports Phase 1b)
def run_full_3d_simulation(state):
    t0 = time.time()
    inputs = {
        'engine_type': state.get('engine_type'),  # <-- NEW: phase 1a / 1b / analytical flag
        'grid': {
        ...
        'pvt': {
            ...
            'a_g': float(state['a_g']), 'z_g': float(state['z_g']),
            'ct_o_1psi': float(state['ct_o_1psi']),
            'ct_g_1psi': float(state['ct_g_1psi']),
            'ct_w_1psi': float(state['ct_w_1psi'])
        },
        'relperm': {
        ...
        'schedule': {
            'control': state['pad_ctrl'],
            'bhp_psi': float(state['pad_bhp_psi']),
            'rate_mscfd': float(state['pad_rate_mscfd'])
        },
        'msw': {
            'laterals': int(state['n_laterals']),
            'L_ft': float(state['L_ft']), 'stage_spacing_ft': float(state['stage_spacing_ft']),
            'clusters_per_stage': int(state['clusters_per_stage']),
            'dp_limited_entry_psi': float(state['dP_LE_psi']),
            'friction_factor': float(state['f_fric']),
            'well_ID_ft': float(state['wellbore_ID_ft']),
            'xf_ft': float(state['xf_ft']), 'hf_ft': float(state['hf_ft']),
            'weights': []
        },
...
def run_simulation(state):
    ...
    if result is None:
        ...
        return fallback_fast_solver(state, np.random.default_rng(int(st.session_state.rng_seed)))
...
with st.sidebar:
    ...
    try:
        # Create a temporary grid object to get info
        temp_grid_inputs = {'grid': {k: st.session_state[k] for k in ['nx', 'ny', 'nz', 'dx', 'dy', 'dz']}}
        from full3d import Grid
        temp_grid = Grid(temp_grid_inputs)
        st.info(f"**{temp_grid.num_cells:,}** total grid cells.")
    except Exception:
        st.warning("Please define valid grid dimensions.")
    st.markdown("---")
    ...
    # (The rest of the sidebar code is identical to your file)
    st.markdown("### Grid (ft)")
...
# freeze sidebar values into a plain dict for the rest of the app
...
if selected_tab == "Setup Preview":
    ...
    with c1:
        ...
        with st.expander("Click for details"):
            ...
            - **Cell Size**: Physical size of each grid cell (ft).
            - **Total Volume**: Bulk reservoir volume.
            - **Facies Style** controls heterogeneity.
            - **Anisotropy** is the kx/ky ratio.
            """)
    ...
    if st.button("Run simulation", type="primary", use_container_width=True):
        with st.spinner("Running full 3D simulation..."):
            # inject engine_type into state for run_full_3d_simulation
            state_with_engine = dict(state)
            state_with_engine['engine_type'] = st.session_state.get('engine_type')
            st.session_state.sim = run_simulation(state_with_engine)
    ...
    if st.session_state.sim:
        ...
        with g1:
            ...
            st.plotly_chart(eur_g_fig, use_container_width=True, theme="streamlit")
        with g2:
            st.plotly_chart(eur_o_fig, use_container_width=True, theme="streamlit")
        ...
        with c1_res:
            gor = np.divide(sim_data['qg'] * 1000, sim_data['qo'], out=np.full_like(sim_data['qg'], np.nan), where=sim_data['qo']>1*_
    ...
    if sim_data is None and st.session_state.get('kx') is None:
        ...
        if data_3d is not None:
            with st.spinner("Generating 3D plot..."):
                ...
                fig3d = go.Figure()
                fig3d.add_trace(go.Isosurface(x=X.flatten(), y=Y.flatten(), z=Z.flatten(),
                                              value=data_3d_ds.flatten(), isomin=isoval, isomax=v_max,
                ...
                if sim_data:
                    ...
                    for i, lat_y in enumerate(lat_rows_y):
                        fig3d.add_trace(go.Scatter3d(x=[0, L_ft], y=[lat_y, lat_y], z=[lat_z, lat_z], mode='lines',
                                                     line=dict(color='white', width=10),
                                                     name='Well Lateral' if i == 0 else '', showlegend=(i==0)))
                fig3d.update_layout(title=f"<b>3D Isosurface for {prop_3d}</b>",
    ...
    if sim_data is None and st.session_state.get('kx') is None:
        ...
        if data_slice_3d is not None:
            ...
            if "k-plane" in plane_slice:
                slice_idx = st.slider("k-index (z-layer)", 0, nz - 1, nz // 2); data_2d = data_slice_3d[slice_idx, :, :]
            elif "j-plane" in plane_slice:
                slice_idx = st.slider("j-index (y-layer)", 0, ny - 1, ny // 2); data_2d = data_slice_3d[:, slice_idx, :]
            else:
                slice_idx = st.slider("i-index (x-layer)", 0, nx - 1, nx // 2); data_2d = data_slice_3d[:, :, slice_idx]
            fig_slice = px.imshow(data_2d, origin="lower", aspect='equal', labels=dict(), color_continuous_scale='viridis')
    ...
    if 'opt_results' in st.session_state and not st.session_state.opt_results.empty:
        ...
        with c2_res:
            st.markdown("##### Optimization Steps (Top 10)")
    ...
    st.markdown("""
    ### Quick Start
    1) Pick a preset and **Apply**. 2) Generate rock volumes. 3) Run simulation. 4) Inspect results and QA tabs.
    """)
    st.markdown("""
    ### About
    Full 3D simulator with implicit engines (Phase 1a/1b) and a fast analytical proxy for previews/sensitivities.
    """)

elif selected_tab == "Solver & Profiling":
    st.header("Solver & Profiling")
    st.info("This tab shows numerical solver settings and performance.")
    ...
    if st.session_state.get("sim") and 'runtime_s' in st.session_state.sim:
        ...
        st.markdown("*Deeper profiling data not returned by the current engine.*")
    else:
