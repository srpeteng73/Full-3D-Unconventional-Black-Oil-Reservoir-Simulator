...
### **Part 2 of 4: Data Processing, Engine Wrappers, and Sidebar UI**

This part contains functions for processing data like DFN (Discrete Fracture Network) files, the main simulation engine wrapper function that prepares and runs the simulation, and the complete definition of the Streamlit sidebar UI for user inputs.
...
def parse_dfn_csv(uploaded_file):
    ...
    for c in req:
        if c not in df.columns:
            raise ValueError(
                "DFN CSV must include columns: x0,y0,z0,x1,y1,z1[,k_mult,aperture_ft]"
            )
    ...
    if "k_mult" in df.columns or "aperture_ft" in df.columns:
        ...
        ap = (
            df["aperture_ft"].to_numpy(float)
            if "aperture_ft" in df.columns
...
with st.sidebar:
    ...
    with tag_col:
        ...
        def _resource_label(name: str) -> str:
            ...
            if (
                "dry gas" in s
                or ("gas" in s and "oil" not in s and "condensate" not in s and "liquids" not in s)
            ):
                return "Gas"
            if "condensate" in s:
                return "Condensate"
    ...
    st.slider(
        "k stdev (mD around 0.02)", 0.0, 0.20, float(st.session_state.k_stdev), 0.01, key="k_stdev"
    )
    st.slider("ϕ stdev", 0.0, 0.20, float(st.session_state.phi_stdev), 0.01, key="phi_stdev")
    st.slider(
        "Anisotropy kx/ky", 0.5, 3.0, float(st.session_state.anis_kxky), 0.05, key="anis_kxky"
    )

    st.markdown("### Faults")
    st.checkbox("Enable fault TMULT", value=bool(st.session_state.use_fault), key="use_fault")
    fault_plane_choice = st.selectbox(
        "Fault plane", ["i-plane (vertical)", "j-plane (vertical)"], index=0, key="fault_plane"
    )
    ...
    st.number_input(
        ...
        value=float(st.session_state.fault_tm),
        step=0.01,
        key="fault_tm",
    )
    ...
    st.number_input(
        "Lateral length (ft)", value=float(st.session_state.L_ft), step=50.0, key="L_ft"
    )
    st.number_input(
        "Stage spacing (ft)",
        value=float(st.session_state.stage_spacing_ft),
        step=5.0,
        key="stage_spacing_ft",
    )
    st.number_input(
        "Clusters per stage",
        1,
        12,
        int(st.session_state.clusters_per_stage),
        1,
        key="clusters_per_stage",
    )
    st.number_input(
        "Δp limited-entry (psi)", value=float(st.session_state.dP_LE_psi), step=5.0, key="dP_LE_psi"
    )
    st.number_input(
        "Wellbore friction factor (pseudo)",
        value=float(st.session_state.f_fric),
        step=0.005,
        key="f_fric",
    )
    st.number_input(
        "Wellbore ID (ft)",
        value=float(st.session_state.wellbore_ID_ft),
        step=0.01,
        key="wellbore_ID_ft",
    )
    st.number_input(
        "Frac half-length xf (ft)", value=float(st.session_state.xf_ft), step=5.0, key="xf_ft"
    )
    st.number_input(
        "Frac height hf (ft)", value=float(st.session_state.hf_ft), step=5.0, key="hf_ft"
    )
    st.slider(
        "Pad interference coeff.",
        0.00,
        0.80,
        float(st.session_state.pad_interf),
        0.01,
        key="pad_interf",
    )
    ...
    st.number_input(
        "Pad BHP (psi)", value=float(st.session_state.pad_bhp_psi), step=10.0, key="pad_bhp_psi"
    )
    st.number_input(
        "Pad RATE (Mscf/d)",
        value=float(st.session_state.pad_rate_mscfd),
        step=1000.0,
        key="pad_rate_mscfd",
    )
    ...
    st.number_input(
        ...
        value=float(st.session_state.p_outer_psi),
        step=10.0,
        key="p_outer_psi",
    )
    ...
    st.checkbox(
        "Use DFN-driven sink in solver",
        value=bool(st.session_state.use_dfn_sink),
        key="use_dfn_sink",
    )
    st.checkbox(
        "Auto-generate DFN from stages when no upload",
        value=bool(st.session_state.use_auto_dfn),
        key="use_auto_dfn",
    )
    st.number_input(
        "DFN influence radius (ft)",
        value=float(st.session_state.dfn_radius_ft),
        step=5.0,
        key="dfn_radius_ft",
    )
    st.number_input(
        "DFN sink strength (psi)",
        value=float(st.session_state.dfn_strength_psi),
        step=10.0,
        key="dfn_strength_psi",
    )
    dfn_up = st.file_uploader(
        "Upload DFN CSV: x0,y0,z0,x1,y1,z1[,k_mult,aperture_ft]", type=["csv"], key="dfn_csv"
    )
    c1, c2 = st.columns(2)
    with c1:
    ...
    st.number_input(
        "Newton tolerance",
        value=float(st.session_state.newton_tol),
        format="%.1e",
        key="newton_tol",
    )
    st.number_input(
        "Transmissibility tolerance",
        value=float(st.session_state.trans_tol),
        format="%.1e",
        key="trans_tol",
    )
    st.number_input(
        "Max Newton iterations", value=int(st.session_state.max_newton), step=1, key="max_newton"
    )
    st.number_input(
        "Max linear solver iterations", value=int(st.session_state.max_lin), step=10, key="max_lin"
    )
    st.number_input(
        "Threads (0 for auto)", value=int(st.session_state.threads), step=1, key="threads"
    )
    st.checkbox(
        "Use OpenMP for parallelism", value=bool(st.session_state.use_omp), key="use_omp"
    )
    st.checkbox(
        "Use Intel MKL for linear algebra", value=bool(st.session_state.use_mkl), key="use_mkl"
    )
    st.checkbox(
        "Use PyAMG algebraic multigrid solver",
        value=bool(st.session_state.use_pyamg),
        key="use_pyamg",
    )
    st.checkbox(
        "Use NVIDIA cuSPARSE (if GPU available)",
        value=bool(st.session_state.use_cusparse),
        key="use_cusparse",
    )
    st.markdown("---")
...
### **Part 3 of 4: Main Application Tabs (A)**

This section begins the layout of the main application window, defining the tab navigation. It includes the code for the "Setup Preview," "Control Panel," "Generate 3D property volumes," "PVT," "MSW Wellbore," "RTA," and the primary "Results" tab.
...
if selected_tab == "Setup Preview":
    ...
    with c2:
        ...
        L_ft, xf_ft, ss_ft, n_lats = (
            state["L_ft"],
            state["xf_ft"],
            state["stage_spacing_ft"],
            state["n_laterals"],
        )
        fig.add_shape(
            type="rect",
    ...
    with p_c1:
        fig_g = go.Figure(
            go.Scatter(x=preview["t"], y=preview["qg"], name="Gas Rate", line=dict(color="#d62728"))
        )
    ...
    with p_c2:
        fig_o = go.Figure(
            go.Scatter(x=preview["t"], y=preview["qo"], name="Oil Rate", line=dict(color="#2ca02c"))
        )
    ...
    with c1:
        ...
        if st.session_state.control == "BHP":
            st.number_input(
                ...
                500.0,
                15000.0,
                float(st.session_state.get("bhp_psi", 2500.0)),
                50.0,
                key="bhp_psi",
            )
            ...
            st.number_input(
                ...
                0.0,
                500000.0,
                float(st.session_state.get("rate_mscfd", 5000.0)),
                100.0,
                key="rate_mscfd",
            )
            ...
            st.number_input(
                ...
                0.0,
                20000.0,
                float(st.session_state.get("rate_stbd", 800.0)),
                10.0,
                key="rate_stbd",
            )
            ...
            st.number_input(
                ...
                0.0,
                40000.0,
                float(st.session_state.get("rate_stbd", 1200.0)),
                10.0,
                key="rate_stbd",
            )
    with c2:
        ...
        st.number_input(
            "kv/kh", 0.01, 1.0, float(st.session_state.get("kvkh", 0.10)), 0.01, "%.2f", key="kvkh"
        )
        st.number_input(
            "Geomech α (1/psi)",
            0.0,
            1e-3,
            float(st.session_state.get("geo_alpha", 0.0)),
            1e-5,
            "%.5f",
            key="geo_alpha",
        )
    ...
    if st.session_state.get("kx") is not None:
        ...
        kx_display, ky_display, phi_display = (
            get_k_slice(st.session_state.kx, state["nz"] // 2),
            get_k_slice(st.session_state.ky, state["nz"] // 2),
            get_k_slice(st.session_state.phi, state["nz"] // 2),
        )
        c1, c2 = st.columns(2)
        with c1:
            st.plotly_chart(
                px.imshow(
                    ...
                    labels=dict(color="mD"),
                    title="<b>kx — mid-layer (mD)</b>",
                ),
        ...
        st.plotly_chart(
            px.imshow(
                ...
                labels=dict(color="ϕ"),
                title="<b>Porosity ϕ — mid-layer (fraction)</b>",
            ),
            use_container_width=True,
        )
    else:
    ...
    P = np.linspace(
        max(1000, state["p_min_bhp_psi"]), max(2000, state["p_init_psi"] + 1000), 120
    )
    Rs, Bo, Bg, mug = (
    ...
    f1.update_layout(
        template="plotly_white",
        title="<b>Solution GOR Rs vs Pressure</b>",
        xaxis_title="Pressure (psi)",
        yaxis_title="Rs (scf/STB)",
    )
    ...
    try:
        ...
        well_id_ft, f_fric, dP_le = (
            float(state["wellbore_ID_ft"]),
            float(state["f_fric"]),
            float(state["dP_LE_psi"]),
        )
        ...
        with c1_msw:
            fig_p = go.Figure(
                go.Scatter(
                    x=np.arange(n_stages) * ss_ft, y=p_wellbore_at_stage, mode="lines+markers"
                )
            )
            fig_p.update_layout(
                title="<b>Wellbore Pressure Profile</b>",
                xaxis_title="Dist. from Heel (ft)",
        ...
        with c2_msw:
            ...
            fig_q.update_layout(
                title="<b>Flow Contribution per Stage</b>",
                xaxis_title="Dist. from Heel (ft)",
    ...
    if sim is None:
            ...
            gfig = go.Figure(
                go.Indicator(
                    ...
                    gauge=dict(
                        ...
                        bar=dict(color="red"),
                        steps=[dict(range=[0, max(1.0, eur_g * 0.7)], color="#f3f3f7")],
                        threshold=dict(
                            line=dict(color="red", width=4), thickness=0.75, value=eur_g
                        ),
                    ),
                )
            )
            ...
            ofig = go.Figure(
                go.Indicator(
                    ...
                    gauge=dict(
                        ...
                        steps=[dict(range=[0, max(0.1, eur_o * 0.7)], color="#f3f3f7")],
                        threshold=dict(
                            line=dict(color="green", width=4), thickness=0.75, value=eur_o
                        ),
                    ),
                )
            )
        ...
        if t is not None and any(v is not None for v in (qg, qo, qw)):
            ...
            fig_rate.update_yaxes(
                title_text="Gas Rate (Mscf/d)", secondary_y=False, showgrid=True, gridwidth=1
            )
            fig_rate.update_yaxes(
                title_text="Liquid Rate (STB/d)", secondary_y=True, showgrid=False
            )
        ...
        if t is not None and (cum_g is not None or cum_o is not None or cum_w is not None):
            ...
            if cum_g is not None:
                fig_cum.add_trace(
                    go.Scatter(
                        x=t, y=cum_g, name="Cum Gas (BCF)", line=dict(color="red", width=2)
                    ),
                    secondary_y=False,
                )
            if cum_o is not None:
                fig_cum.add_trace(
                    go.Scatter(
                        x=t, y=cum_o, name="Cum Oil (MMbbl)", line=dict(color="green", width=2)
                    ),
                    secondary_y=True,
                )
            ...
            fig_cum.update_yaxes(
                title_text="Gas (BCF)", secondary_y=False, showgrid=True, gridwidth=1
            )
...
### **Part 4 of 4: Main Application Tabs (B) and Advanced Features**

This final part includes the more advanced and analytical tabs: the 3D and Slice Viewers, Quality Assurance and Material Balance, Economics, Field Matching, Monte Carlo uncertainty analysis, Well Placement Optimization, and the User Manual.
...
```python
    ...
    with st.spinner("Generating 3D plot..."):
        ...
        fig3d.update_layout(
            ...
            scene=dict(
                xaxis_title="X (ft)", yaxis_title="Y (ft)", zaxis_title="Z (ft)", aspectmode="data"
            ),
    ...
    if sim_data is None and st.session_state.get("kx") is None:
        ...
        with c2:
            plane_slice = st.selectbox(
                "Select plane:", ["k-plane (z, top-down)", "j-plane (y, side-view)", "i-plane (x, end-view)"]
            )
        data_3d = (
            st.session_state.get("kx")
            if "kx" in prop_slice
            else st.session_state.get("ky")
            if "ky" in prop_slice
            else st.session_state.get("phi")
            if "ϕ" in prop_slice
        ...
        if data_3d is not None:
            ...
            fig = px.imshow(
                data_2d, origin="lower", aspect="equal", labels=labels, color_continuous_scale="viridis"
            )
    ...
    if not all(k in sim for k in ("t", "qg", "qo")) or len(sim["t"]) < 2:
        st.warning(
            "Simulation data is missing required arrays ('t', 'qg', 'qo') for this analysis."
        )
        st.stop()
    ...
    c2.metric(
        "Material Balance GIIP (from P/Z)",
        f"{giip_bcf:.2f} BCF",
        delta=(
            f"{(giip_bcf - sim_eur_g_bcf)/sim_eur_g_bcf:.1%} vs Sim" if sim_eur_g_bcf > 0 else None
        ),
    )
    ...
    fig_pz_gas.add_trace(
        go.Scatter(
            x=x_fit, y=y_fit, mode="lines", name="Linear Extrapolation", line=dict(dash="dash")
        )
    )
    ...
    if st.session_state.get("sim") is None:
        ...
        qw = np.nan_to_num(
            np.asarray(sim.get("qw")) if sim.get("qw") is not None else np.zeros_like(qo),
            nan=0.0,
        )
        ...
        fig_cf.update_layout(
            template="plotly_white",
            title_text="<b>Cash Flow (Approx.)</b>",
            xaxis_title="Time (days)",
            yaxis_title="k$",
        )
    ...
    with c2:
        ...
        if st.button("Load Demo Data", use_container_width=True):
            ...
            demo_df = pd.DataFrame(
                {"Day": days, "Gas_Rate_Mscfd": gas_rate, "Oil_Rate_STBpd": oil_rate}
            )
    ...
    if "field_data_match" in st.session_state:
        ...
        if st.session_state.get("sim") is not None and st.session_state.get(
            "field_data_match"
        ) is not None:
            ...
            if {"Day", "Oil_Rate_STBpd"}.issubset(field_data.columns):
                fig_match.add_trace(
                    go.Scatter(
                        x=field_data["Day"],
                        y=field_data["Oil_Rate_STBpd"],
                        mode="markers",
                        name="Field Oil",
                        marker=dict(color="#2ca02c", symbol="cross"),
                        yaxis="y2",
                    )
                )
            ...
            layout_config.update(
                ...
                yaxis2=dict(
                    title="Oil Rate (STB/d)", overlaying="y", side="right", showgrid=False
                ),
            )
            ...
            st.session_state.get("sim") is None
            and st.session_state.get("field_data_match") is not None
        ):
            st.info(
                "Demo/Field data loaded. Run a simulation on the 'Results' tab to view the comparison plot."
            )
    ...
    if st.session_state.get("field_data_match") is None:
        ...
        to_vary = st.multiselect(
            "Choose parameters to vary:", candidates, default=["k_stdev", "xf_ft"]
        )
        bounds = {}
        ...
        if st.button("Run Automated Match", type="primary"):
            ...
            def misfit(x):
                ...
                if {"Day", "Oil_Rate_STBpd"}.issubset(field_data.columns):
                    ...
                    idxs = np.clip(
                        np.searchsorted(res["t"], fd["Day"].values), 0, len(res["t"]) - 1
                    )
            ...
            if algo == "Differential Evolution":
                ...
                result = minimize(
                    misfit, x0, method="Nelder-Mead", options={"maxiter": int(max_iter)}
                )
                st.success(f"Best misfit: {result.fun:.4g}")
    ...
    if "mc_results" in st.session_state:
        ...
        with c1:
            fig = go.Figure(
                [
                    go.Scatter(
                        x=mc["t"], y=p90_g, fill=None, mode="lines", line_color="lightgrey", name="P10"
                    ),
                    go.Scatter(
                        x=mc["t"],
                        y=p10_g,
            ...
            st.plotly_chart(
                fig.update_layout(
                    **semi_log_layout("Gas Rate Probabilistic Forecast", yaxis="Gas Rate (Mscf/d)")
                ),
                use_container_width=True,
                theme="streamlit",
            )
        ...
        with c2:
            ...
            st.plotly_chart(
                fig.update_layout(
                    **semi_log_layout("Oil Rate Probabilistic Forecast", yaxis="Oil Rate (STB/d)")
                ),
                use_container_width=True,
                theme="streamlit",
            )
    ...
    if launch_opt:
        ...
        for i in range(int(iterations)):
            ...
            if not is_valid:
                st.error(
                    "Could not find a valid heel location. Check grid size, L_ft, and fault settings."
                )
                break
            temp_state = base_state.copy()
    ...
    if "opt_results" in st.session_state and not st.session_state.opt_results.empty:
        ...
        with c1_res:
            ...
            score_unit = (
                "MMBO"
                if "Oil" in st.session_state.get("opt_objective", "Maximize Oil EUR")
                else "BCF"
            )
        ...
        fig_opt.add_trace(
            go.Scatter(
                ...
                mode="markers",
                marker=dict(
                    color="cyan", size=16, symbol="star", line=dict(width=2, color="black")
                ),
                name="Best Location",
            )
        )
