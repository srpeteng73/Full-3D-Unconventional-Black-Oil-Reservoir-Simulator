...
from core.blackoil_pvt1 import BlackOilPVT   # <- not OilPVT/GasPVT/WaterPVT
...
class Assembler:
    ...
    # neighbor list of (c, n, Tx, Ty, Tz) harmonic-avg directional T
    ...
    def __post_init__(self):
        ...
        # wells controlled by RATE (extra DOFs)
    ...
    def _build_neighbors(self):
        ...
        nbrs: list[tuple[int, int, float, float, float]] = []
        ...
        for k in range(g.nz):
            for j in range(g.ny):
                for i in range(g.nx - 1):
                    ...
                    kx = self._harm(g.kx[c], g.kx[n])
                    ky = self._harm(g.ky[c], g.ky[n])
                    kz = self._harm(g.kz[c], g.kz[n])
                    T = conv * area / dist
                    nbrs.append((c, n, T * kx, T * ky, T * kz))
    ...
    def residual_and_jacobian(self, x_n: np.ndarray, x_prev: np.ndarray, dt_days: float):
        ...
        # PVT and relperm + derivatives (pvt1/relperm1 must provide these)
        ...
        phi = self.grid.phi  # grid1 should expose phi as an array
        ...
        for c, nbh, Tx, Ty, Tz in self.neighbors:
            # Use Tx as scalar T (kx-based). You can blend Tx/Ty/Tz if you add directionality later.
            T = Tx
            dP = Pn[c] - Pn[nbh]
